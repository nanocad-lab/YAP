...
import numpy as np
from overlay_yield_calculator import _contact_area_misalignment_limit
...
def die_pad_misalignment(
    ...
    if approximate_set != 1:
        ...
        # A = np.array([[system_magnification_ppm, -system_rotation_rad], [system_rotation_rad, system_magnification_ppm]])
        # offset = np.array([system_translation_x_um, system_translation_y])
...
def overlay_term_simulator(
    ...
    return system_translation_x_um, system_translation_y_um, system_rotation_rad, system_magnification_ppm, MAX_ALLOWED_MISALIGNMENT
