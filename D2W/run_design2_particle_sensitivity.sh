...
BASE_CONFIG="configs/design_2/design_2.yaml"
OUT_TSV="${OUT_TSV:-output/particle_sensitivity_design_2.tsv}"
...
make_config() {
  ...
  local cfg="configs/design_2/design_2__${suffix}.yaml"
  cp "$BASE_CONFIG" "$cfg"
  python - <<PY
import re
  ...
  for d1 in "${D1_LIST[@]}"; do
    for width in "${WIDTH_LIST[@]}"; do
      ...
      for variant in "${VARIANTS[@]}"; do
        ds_name="design_2/${RATIO}/${variant}"
        ds_dir="input/design_2/${RATIO}/${variant}"
