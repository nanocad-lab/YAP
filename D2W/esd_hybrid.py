...
def plot_probability_over_pads_with_pitch(
    ...
    *,
    ...
    ax.set_xlim(-TOP_DIE_W_UM / 2.0, TOP_DIE_W_UM / 2.0)
    ax.set_ylim(-TOP_DIE_H_UM / 2.0, TOP_DIE_H_UM / 2.0)
    ax.invert_yaxis()
    ax.set_title(title)
...
if __name__ == "__main__":
    if not PAD_COORDS_UM:
        xs = np.arange(-TOP_DIE_W_UM / 2.0 + PAD_PITCH_UM * 0.5, TOP_DIE_W_UM / 2.0, PAD_PITCH_UM)
        ys = np.arange(TOP_DIE_H_UM / 2.0 - PAD_PITCH_UM * 0.5, -TOP_DIE_H_UM / 2.0, -PAD_PITCH_UM)
