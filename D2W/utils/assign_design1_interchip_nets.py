...
def build_assignment_order(path: Path, entries: list[list[str]]) -> list[int]:
    ...
    if mode == "center":
        ...
        seed = int.from_bytes(
            hashlib.sha256(str(path).encode("utf-8")).digest()[:8],
            "big",
        )
...
def retag_pair(
    ...
    compute_order = build_assignment_order(compute_path, compute_entries)
    memory_order = build_assignment_order(memory_path, memory_entries)
