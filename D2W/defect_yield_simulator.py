...
def defect_yield_simulator(
    ...
    def generate_particles(particle_thickness, DIE_W, DIE_L, drop_particle_range):
        particles = []
        for i in range(len(particle_thickness)):
            # x, y = np.random.uniform(-DIE_W / 2, DIE_W / 2), np.random.uniform(-DIE_L / 2, DIE_L / 2)
            x, y = np.random.uniform(-DIE_W / 2 * drop_particle_range, DIE_W / 2 * drop_particle_range), np.random.uniform(-DIE_L / 2 * drop_particle_range, DIE_L / 2 * drop_particle_range)
    ...
    total_particles = (drop_particle_range * DIE_W) * (drop_particle_range * DIE_L) * D0 * NUM_DIES     # Put the particles on the 2*DIE_W * 2*DIE_L area
    ...
    for die_ind in range(NUM_DIES):
        ...
        particles = generate_particles(particle_thickness, DIE_W, DIE_L, drop_particle_range)
